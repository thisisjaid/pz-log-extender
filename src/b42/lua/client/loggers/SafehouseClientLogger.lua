...
-- B41 bug fix: was incorrectly using global LogExtenderClient instead of SafehouseClientLogger.
SafehouseClientLogger.OnGameStart = function()
    if SandboxVars.LogExtender.SafehouseAdminTools then
        SafehouseClientLogger.OnAddSafeHouse()
    end
end

Events.OnGameStart.Add(SafehouseClientLogger.OnGameStart);
