...
-- B42: sendClientCommand requires the player object as the first argument.
function logutils.WriteLog(filemask, message)
    sendClientCommand(getPlayer(), "LogExtender", "write", { mask = filemask, message = message });
end
...
-- B42: player:getTraits() removed; iterate CharacterTraitDefinition and check hasTrait().
function logutils.GetPlayerTraits(player)
    ...
    for i = 0, allTraits:size() - 1 do
        ...
        if traitDef then
            local traitEnum = traitDef:getType();
            if player:hasTrait(traitEnum) then
                table.insert(traits, '"' .. tostring(traitEnum) .. '"');
            end
        end
    end
...
function logutils.GetPlayerStats(player)
    ...
    if descriptor then
        local charProf = descriptor:getCharacterProfession();
        if charProf then
